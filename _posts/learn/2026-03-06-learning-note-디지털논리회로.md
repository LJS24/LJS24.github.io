...
디지털 회로에서는 단순히 `0`과 `1`이라는 한 점으로 판단하는 것이 아니라, 각 값을 의미하는 `전압 범위`를 기준으로 신호를 해석한다. 이때 출력 신호의 진폭 범위가 더 좁은데, 출력 신호의 진폭 범위가 더 좁아질 수 있는 이유는 신호 전달 과정에서 에너지 손실이 발생하기 때문이다.
